...
- Los cuatro datos aún abiertos - top ventas, ticket promedio, capacidad diaria y contactos B2B - no bloquean repositorio, arquitectura, CMS, componentes, staging ni desarrollo inicial. Solo bloquean la carga comercial definitiva, ciertas campañas y el lanzamiento cuando sean necesarios.
...
|---|---|
| `/` | Home principal |
| `/comprar-flores` | Hub de catálogo y descubrimiento |
| `/bouquets/` | URL indexada actual preservada |
| `/rosas/` | URL indexada actual preservada |
| `/topiarios/` | URL indexada actual preservada |
| `/exoticos/` | URL indexada actual preservada |
| `/fruteros/` | URL indexada actual preservada |
| `/cajas-florales/` | Categoría |
| `/ramos-buchones/` | Categoría |
| `/tropicales/` | Categoría |
| `/condolencias/` | Categoría sensible |
| `/arreglos-corporativos/` | Línea empresarial |
...
- Las URLs indexadas actuales deben mantenerse exactamente como están:
  - `/rosas/`
  - `/topiarios/`
  - `/exoticos/`
  - `/fruteros/`
  - `/bouquets/`
- `/comprar-flores/` funciona solo como hub y no debe mover esas rutas dentro de su jerarquía salvo que exista una razón técnica aprobada y un mapa 301 explícito.
- Si algún slug nuevo cambia, se registra en `redirect-map.csv` y se publica con 301.
...
- Registrar los cuatro datos abiertos como pendientes de validación comercial.
- Esta fase no depende de que los cuatro datos abiertos estén resueltos para comenzar la arquitectura.
...
- Los cuatro datos abiertos no bloquean esta fase.
...
- Esta fase puede avanzar con catálogo parcial mientras se completan los datos pendientes.
...
- Aquí solo deben quedar resueltos los datos que afecten la carga comercial definitiva, las campañas que los necesiten y el lanzamiento final.
...
- El lanzamiento final no se realiza si siguen faltando top ventas, ticket promedio, capacidad diaria o contactos B2B para las partes del sitio o campañas que los requieran.
...
- Esta fase también depende de datos operativos completos para evitar sobreventa y definir catálogo comercial real.
...
- Los cuatro datos abiertos ya mencionados, solo para la carga comercial definitiva, campañas y lanzamiento.
...
- La implementación técnica puede arrancar con los accesos mínimos y sin esperar los cuatro datos abiertos.
- Los cuatro datos abiertos solo deben estar completos antes de publicar la carga comercial definitiva, activar campañas que dependan de ellos y cerrar el lanzamiento.
